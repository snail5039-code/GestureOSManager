...
// 1) Push latest saved settings to agent on connect (best-effort)
try {
  controlService.updateSettings(settingsService.getSettings());
} catch (Exception ignore) {
}

// 2) ✅ Make input work immediately:
// - enable agent (equivalent to clicking "Start")
// - clear UI lock to avoid "enabled but locked" dead state
// NOTE: All are best-effort; if agent disconnects immediately, we'll just fail silently.
try {
  controlService.setUiLock(false);
} catch (Exception ignore) {
}
try {
