...
for i in range(start_idx, len(lines)):
    if 'if chance_requested and not chance_consumed:' in lines[i] and i > start_idx + 100:
        # This might be the timeout block at the end
        pass
...
new_logic = """            # 🔒 Chance Time Logic (Guard does NOT block chance)
            if chance_requested and not chance_consumed:
                # 1️⃣ READY PHASE: Neutral Captured
                if chance_phase == "ready":
                    ...
                    # Forward move check only (Z movement)
                    ...
                    if ((dz_n < -z_enter_thresh or dy_n < uppercut_y_thresh)
                        ...
                        if ready_to_active_counter >= 2:  # 연속 2프레임 필요
                            chance_phase = "active"
                            intent_counter = 0
                ...
                # 2️⃣ ACTIVE PHASE: Judge Attack
                    ...
                    # 🔒 손 들고 정지 상태 차단 (Hook 예외: dx가 크면 정지 아님)
                    if total_move < 0.07 and speed < 0.02 and abs(dx) < 0.10:
                        intent_counter = 0
                    ...
                    if total_move < 0.02:
                        pass
                    elif total_move < 0.025 and speed < 0.012:
                        intent_counter = max(0, intent_counter - 1)
                    
                    # 🔒 Hand leads body check
                    ...
                    # 🔒 Intent 판단 기준
                    MIN_TOTAL_MOVE = 0.065
                    MIN_SPEED = 0.018
                    if abs(dx) > 0.15:  # Hook 예외
                        MIN_TOTAL_MOVE = 0.045
                        MIN_SPEED = 0.012

                    base_intent = total_move > MIN_TOTAL_MOVE and speed > MIN_SPEED and hand_leads
                    ...
                    # 🔥 최소 3프레임 이상 공격 패턴 누적 필요
                    if chance_phase == "active" and intent_counter >= REQUIRED_INTENT_FRAMES:
                        attack_type = "none"

                        # 🔒 HARD BLOCK: 거의 정지 상태면 공격 없음
                        if total_move < 0.065 or speed < 0.018:
                            attack_type = "none"
                        # Hook/uppercut 예외
                        elif abs(dx) > 0.15 or dy < -0.12:
                            pass

                        # 🔹 공격 우선순위
                        # 1️⃣ Uppercut
                        if dy < -0.14 and abs(dz) < 0.06 and abs(dx) < 0.20 and speed > 0.02 and total_move > 0.10:
                            attack_type = "uppercut"
                        # 2️⃣ Hook
                        elif abs(dx) > 0.18 and abs(dy) < 0.07 and abs(dz) < 0.06 and speed > 0.02 and total_move > 0.10:
                            attack_type = "hook"
                        # 3️⃣ Straight
                        elif dz <= -0.13 and total_move > 0.11 and abs(dy) < 0.06 and abs(dx) < 0.20 and speed > 0.02:
                        ...
                        if attack_type != "none":
                            ...
                            reset_chance_fsm()  # 단일 공격 즉시 종료
                        elif chance_phase == "active":
                            intent_counter = max(0, intent_counter - 1)
                            active_ambiguous_counter += 1
                            MAX_AMBIGUOUS = 8
                            if active_ambiguous_counter > MAX_AMBIGUOUS:
                                print("[FSM] AMBIGUOUS -> FAIL -> EXIT CHANCE")
                                socketio.emit("motion", {"dir": "fail", "t": time.time()})
                                reset_chance_fsm()

                    # 🔒 ACTIVE Time Limit (0.9s)
                    if chance_phase == "active" and now_t - active_enter_time > 0.9:
                ...
                # 3️⃣ CONSUMED PHASE: Block Everything
...
print("Successfully updated boxing_controller.py")
