...
def _force_topmost(hwnd_int: int):
    ...
    try:
        ...
        try:
            user32.BringWindowToTop(wintypes.HWND(hwnd_int))
        except Exception:
            pass
    except Exception:
        pass
...
def run_menu_process(cmd_q, evt_q):
    ...
    class MenuWindow(QtWidgets.QWidget):
        def __init__(self):
            ...
            # ✅ topmost 재강제 타이머
            self._topmost_last = 0.0
            self._topmost_period = 0.20  # 0.2s마다 강제
        ...
        def _ensure_topmost(self, force: bool = False):
            nowt = time.time()
            if (not force) and (nowt - self._topmost_last) < self._topmost_period:
                return
            self._topmost_last = nowt
        ...
        def setActive(self, on: bool):
            ...
            if on and (not self._active):
                ...
                # ✅ OSK/키보드가 TOPMOST라도, show+raise 직후 즉시 재강제
        ...
        def tick(self, dt: float):
            ...
            if raw is not None:
                ...
                self._last_nonnull_t = nowt

            if raw is None and self._last_nonnull is not None:
                if (nowt - self._last_nonnull_t) <= HOLD_SECONDS:
                    raw = self._last_nonnull

            if raw != self._cand:
                self._cand = raw
                self._cand_since = nowt
            else:
                if (nowt - self._cand_since) >= DEBOUNCE_SECONDS:
                    if raw != self._hover:
                        self._hover = raw
                        self._emit(raw)

            # ✅ OSK가 topmost 유지해도, 메뉴도 계속 topmost 재강제
            if self._active:
                self._ensure_topmost(force=False)
    ...
    def pump_cmd():
        ...
        # OS가 날리는 경우가 있어서 주기적으로 재적용(추가 안전망)
        if int(nowt) % 5 == 0:
            try:
                _apply_win_exstyle(int(win.winId()), click_through=True)
                _force_topmost(int(win.winId()))
