...
for i, line in enumerate(lines):
    if '# HARD BLOCK' in line and 'if total_move < 0.065 or speed < 0.018:' in line:
        start_idx = i
        break

if start_idx == -1:
    # Try another pattern if the previous one was already updated
    for i, line in enumerate(lines):
        if '# HARD BLOCK' in line and 'total_move < 0.065' in line:
            start_idx = i
            break
...
# Find the end of the classification block (before the final_attack assignment)
end_idx = -1
