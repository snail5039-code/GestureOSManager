...
# =============================================================================
# Windows: avoid flashing cmd windows when launching OSK / TabTip / URIs
# =============================================================================
try:
    _IS_WIN = (os.name == "nt")
except Exception:
...
if _IS_WIN:
    try:
        ...
        _ShellExecuteW.argtypes = [wintypes.HWND, wintypes.LPCWSTR, wintypes.LPCWSTR, wintypes.LPCWSTR, wintypes.LPCWSTR, ctypes.c_int]
        _ShellExecuteW.restype = wintypes.HINSTANCE
        _SW_SHOWNORMAL = 1
        _SW_HIDE = 0

        def _shell_open(target: str) -> bool:
            # returns True on success (hinst > 32)
            try:
                h = _ShellExecuteW(None, "open", target, None, None, _SW_HIDE)
                return int(h) > 32
            except Exception:
                return False
        ...
        if not launched:
            try:
                p = subprocess.Popen(
                    ...
                    stderr=subprocess.DEVNULL,
                    shell=False,
                    creationflags=(subprocess.CREATE_NO_WINDOW if _IS_WIN and hasattr(subprocess, 'CREATE_NO_WINDOW') else 0),
                )
                self._osk_proc = p
                time.sleep(0.12)
                if _tasklist_has("osk.exe"):
