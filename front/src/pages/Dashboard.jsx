...
// 백엔드 mode 값과 맞추세요 (현재 네 STATUS JSON에 mode:"MOUSE"가 보임)
...
function IconGrid() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-sky-200">
            <path
...
export default function Dashboard() {
    ...
    const derived = useMemo(() => {
        const s = status || {};
        return {
            connected: !!s.connected, // 백엔드가 필드 제공 안 하면 false로 표시됨
            enabled: !!s.enabled,
            locked: !!s.locked,
    ...
    const applyMode = async (nextMode) => {
        setMode(nextMode);       // UI 먼저 반영(원하면 이 줄 빼도 됨)
        setBusy(true);
        setError("");
        try {
            // 컨트롤러: @RequestParam String mode 이므로 params로 보냄
            await api.post("/control/mode", null, { params: { mode: nextMode } });
            await fetchStatus();   // 서버 상태 다시 가져오기
        } catch (e) {
            const msg = e?.response
        ...
        <div className="min-h-screen bg-[#0b1220] text-slate-100">
            ...
            <div className="sticky top-0 z-20 border-b border-white/10 bg-gradient-to-r from-[#0b4aa2]/40 via-[#0b1220]/80 to-[#0b1220]/80 backdrop-blur">
                <div className="mx-auto max-w-[1200px] px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-base font-bold tracking-tight">Gesture Agent Manager</div>
                            <div className="text-xs text-slate-300/80">
                                REST polling {POLL_MS}ms · {lastUpdated ? `Last: ${lastUpdated.toLocaleTimeString()}` : "—"}
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-2">
                        {error ? <Badge tone="red">ERROR</Badge> : <Badge tone="green">OK</Badge>}
                        <Badge tone={derived.enabled ? "green" : "slate"}>{derived.enabled ? "ENABLED" : "DISABLED"}</Badge>
                        <Badge tone={derived.locked ? "yellow" : "slate"}>{derived.locked ? "LOCKED" : "UNLOCKED"}</Badge>
                        <Badge tone={preview ? "blue" : "slate"}>{preview ? "PREVIEW" : "NO PREVIEW"}</Badge>
            ...
            <div className="mx-auto max-w-[1200px] px-6 py-6 grid grid-cols-12 gap-5">
                ...
                <aside className="col-span-12 lg:col-span-3 space-y-5">
                            ...
                            {[
                                ...
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        it.active ? "bg-sky-500/15 ring-1 ring-sky-400/20" : "hover:bg-white/5"
                                    )}
                                >
                                    <div className="flex items-center gap-3">
                                        <span className={cn("h-2.5 w-2.5 rounded-full", it.active ? "bg-sky-300" : "bg-slate-600")} />
                    ...
                    <Card title="Quick Actions" right={busy ? <Badge tone="blue">BUSY</Badge> : <Badge tone="slate">READY</Badge>}>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                            <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                ...
                                <div className="mt-1 font-semibold">{formatNum(derived.fps, 1)}</div>
                            </div>
                            <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                        ...
                        <div className="mt-4 text-xs text-slate-400">
                            Start/Stop: <span className="text-slate-200">POST /api/control/start</span>,{" "}
                ...
                <main className="col-span-12 lg:col-span-9 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <Card title="Mode">
                            ...
                            <div className="mt-4 text-xs text-slate-400">
                                Mode 변경: <span className="text-slate-200">POST /api/control/mode</span> {"{ mode }"}
                            </div>
                        </Card>

                        <Card title="Connection" right={loading ? <Badge tone="yellow">LOADING</Badge> : <Badge tone="slate">LIVE</Badge>}>
                            <div className="grid grid-cols-2 gap-3 text-sm">
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                    <div className="text-xs text-slate-400">connected</div>
                                    <div className="mt-1 font-semibold">{String(derived.connected)}</div>
                                </div>
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                <Badge tone={error ? "red" : "green"}>{error ? "DEGRADED" : "HEALTHY"}</Badge>
                            ...
                            <div className="mt-4 text-xs text-slate-400">
                                connected/fps는 백엔드가 내려주는 필드명에 따라 표시(없으면 기본값).
                            </div>
                        </Card>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <Card title="Agent State">
                            <div className="grid grid-cols-2 gap-3 text-sm">
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                    ...
                                    <div className="mt-1 font-semibold">{String(derived.enabled)}</div>
                                </div>
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                    <div className="text-xs text-slate-400">locked</div>
                                    <div className="mt-1 font-semibold">{String(derived.locked)}</div>
                                </div>
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                    <div className="text-xs text-slate-400">canMove</div>
                                    <div className="mt-1 font-semibold">{derived.canMove === null ? "-" : String(derived.canMove)}</div>
                                </div>
                                <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 p-3">
                                    <div className="text-xs text-slate-400">canClick</div>
                                    <div className="mt-1 font-semibold">{derived.canClick === null ? "-" : String(derived.canClick)}</div>
                                </div>
                            </div>

                            <div className="mt-4 flex flex-wrap gap-2">
                                <Badge tone="slate">gesture: {derived.gesture}</Badge>
                                <Badge tone={derived.enabled ? "green" : "slate"}>enabled: {String(derived.enabled)}</Badge>
                                <Badge tone={derived.locked ? "yellow" : "slate"}>locked: {String(derived.locked)}</Badge>
                            </div>
                        </Card>
                        ...
                        <Card title="Containers (Docker-style list)">
                            <div className="rounded-xl bg-slate-950/50 ring-1 ring-white/10 overflow-hidden">
                                ...
                                <div className="grid grid-cols-12 px-4 py-3 text-sm items-center">
                                    <div className="col-span-6 flex items-center gap-3">
                                        <span className={cn("h-2.5 w-2.5 rounded-full", derived.enabled ? "bg-emerald-400" : "bg-slate-600")} />
                                        <div>
                                            <div className="font-semibold">gesture-agent</div>
                                            <div className="text-xs text-slate-400">mode: {derived.mode}</div>
                                        </div>
                                    </div>

                                    <div className="col-span-3 flex items-center gap-2">
                                        <Badge tone={derived.enabled ? "green" : "slate"}>{derived.enabled ? "running" : "stopped"}</Badge>
                                        <Badge tone={derived.locked ? "yellow" : "slate"}>{derived.locked ? "locked" : "unlocked"}</Badge>
                                    </div>

                                    <div className="col-span-3 text-right">
                                        <div className="text-xs text-slate-400">fps</div>
                                        <div className="font-semibold">{formatNum(derived.fps, 1)}</div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-12 px-4 py-3 text-sm items-center border-t border-white/10">
                                    <div className="col-span-6 flex items-center gap-3">
                                        <span className={cn("h-2.5 w-2.5 rounded-full", derived.scrollActive ? "bg-sky-300" : "bg-slate-600")} />
                                        <div>
                                            <div className="font-semibold">scroll-module</div>
                                            <div className="text-xs text-slate-400">gesture: FIST (assist hand)</div>
                                        </div>
                                    </div>
                            ...
                            <div className="mt-4 text-xs text-slate-400">
                                발표용으로 “도커 리스트 느낌” 내기 위한 더미 UI. 실제 컨테이너가 아니라 상태 기반 표시임.
                            </div>
                        </Card>
                    </div>

                    {/* Raw JSON */}
                    <Card title="Raw Status JSON" right={loading ? <Badge tone="yellow">LOADING</Badge> : <Badge tone="slate">LIVE</Badge>}>
