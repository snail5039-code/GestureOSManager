...
// ✅ file:// 패키징에서도 /api 호출이 살아있게 요청별(8080/8082) 라우팅되는 axios
import { accountApi } from "./api/accountClient";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    accountApi
      .post("/hud/show", null, { params: { enabled: osHudOn ? "true" : "false" } })
      .catch(() => {});
  }, [osHudOn]);
  ...
  const savePairingName = async (nextName) => {
    ...
    try {
      await accountApi.post("/pairing", { name }, { headers: { "Content-Type": "application/json" } });
    } catch {}
    refreshPairing();
  };
  ...
  const savePairingPc = async (nextPc) => {
    ...
    try {
      await accountApi.post("/pairing", { pc }, { headers: { "Content-Type": "application/json" } });
    } catch {}
    refreshPairing();
  };
