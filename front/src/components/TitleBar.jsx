...
function ToggleRow({ title, desc, checked, onToggle }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl ring-1 ring-base-300/45 bg-base-100/12 px-4 py-3">
      ...
      <input type="checkbox" className="toggle toggle-sm" checked={!!checked} onChange={onToggle} />
    </div>
  );
}
    ...
    >
      <div
        className={cn(
          ...
          active ? "bg-primary/10 ring-primary/25" : "bg-base-100/10 ring-base-300/45"
        )}
      >
        {icon}
      </div>
      ...
      <div className="min-w-0">
        <div className="text-sm font-semibold whitespace-nowrap">{title}</div>
        {sub ? <div className="text-[11px] opacity-70 truncate">{sub}</div> : null}
      </div>

      {active ? (
        <span className="ml-auto text-[11px] font-semibold opacity-70 whitespace-nowrap">현재</span>
      ) : (
...
export default function TitleBar({
  ...
  const [settingsPos, setSettingsPos] = useState({ top: 48, left: 0, width: 860 });
  ...
  const calcSettingsPos = () => {
    ...
    // ✅ 설정 모달: 더 크게, 더 쓰기 편하게
    const ideal = Math.min(980, Math.max(760, Math.round(window.innerWidth * 0.78)));
    const width = Math.max(640, Math.min(ideal, window.innerWidth - margin * 2));
  ...
  useEffect(() => {
    ...
    const onDown = (e) => {
      const t = e.target;
      if (safeContains(btnRef.current, t) || safeContains(popRef.current, t)) return;
      setOpen(false);
    };
  ...
  useEffect(() => {
    ...
    const onDown = (e) => {
      const t = e.target;
      if (safeContains(gearBtnRef.current, t) || safeContains(settingsPopRef.current, t)) return;
  ...
  useEffect(() => {
    ...
    const onDown = (e) => {
      const t = e.target;
      if (safeContains(menuBtnRef.current, t) || safeContains(menuPopRef.current, t)) return;
      setMenuOpen(false);
    };
        ...
        >
          ...
          <div className="p-4 border-b border-base-300/45 bg-base-100/8">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <TabBtn active={settingsTab === "display"} onClick={() => setSettingsTab("display")}>
                표시 설정
              </TabBtn>
              <TabBtn active={settingsTab === "motion"} onClick={() => setSettingsTab("motion")}>
                모션 세팅
              </TabBtn>
            </div>
          </div>
          ...
          {settingsTab === "display" ? (
            <div className="flex-1 min-h-0 overflow-auto p-4">
              ...
              <div className="mt-4 text-[11px] opacity-60">
                팁: HUD는 CPU/GPU 부하가 생길 수 있으니 필요할 때만 켜는 것을 권장합니다.
              </div>
            </div>
          ) : (
    ...
    >
      ...
      <div className="ml-auto flex items-center gap-2" style={{ WebkitAppRegion: "no-drag" }}>
        ...
        <button
          ...
          title="테마"
        >
          <span className="text-[11px] opacity-70 mr-2 whitespace-nowrap">Theme</span>
          <span className="text-xs font-semibold whitespace-nowrap">{currentThemeLabel}</span>
        ...
        <div className="flex items-center gap-2">
          <button className="w-10 h-8 rounded-md hover:bg-base-300/30" onClick={onMin} title="Minimize">
            —
          </button>
          <button className="w-10 h-8 rounded-md hover:bg-base-300/30" onClick={onMax} title="Maximize">
            □
          </button>
          <button className="w-10 h-8 rounded-md hover:bg-error/20" onClick={onClose} title="Close">
            ×
          </button>
        </div>
      </div>
